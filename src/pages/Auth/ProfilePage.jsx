...
const PIE_COLORS = [COLORS.blue, COLORS.purple, COLORS.green, COLORS.amber, COLORS.pink];
...
export default function ProfilePage() {
  ...
  const handleLogout = async () => {
    ...
    catch (error) { setLogoutError(error.response?.data?.message || "Unable to log out right now."); }
  ...
  const avgPct = (arr) => !arr.length ? 0 : Math.round(arr.reduce((s, [, v]) => s + (v.completionPercentage || 0), 0) / arr.length);
  ...
  // ── Badges ──────────────────────────────────────────────────────────────────
  const badges = [
    { icon: "🧠", title: "Logic Master",    desc: "Solve 20+ problems",        earned: solvedCount >= 20,       progress: Math.min((solvedCount / 20) * 100, 100),       rarity: "rare" },
    { icon: "⚡", title: "Circuit Creator", desc: "Visit Circuit Forge",        earned: recentEvents.some(e => e.type === "topic_opened"), progress: recentEvents.some(e => e.type === "topic_opened") ? 100 : 0, rarity: "common" },
    { icon: "🗺️", title: "K-Map Pro",       desc: "Complete K-Map topics",      earned: topicStats.kmap >= 80,   progress: topicStats.kmap,                               rarity: "epic" },
    { icon: "🔥", title: "Streak Keeper",   desc: "Maintain a 7-day streak",    earned: streakCurrent >= 7,      progress: Math.min((streakCurrent / 7) * 100, 100),      rarity: "rare" },
    { icon: "🏆", title: "Topic Champion",  desc: "Complete 3+ topics",         earned: completedTopics >= 3,    progress: Math.min((completedTopics / 3) * 100, 100),    rarity: "common" },
    { icon: "🎯", title: "Problem Solver",  desc: "Attempt 10+ problems",       earned: attemptedCount >= 10,    progress: Math.min((attemptedCount / 10) * 100, 100),    rarity: "common" },
    { icon: "🌟", title: "Quiz Champion",   desc: "Solve 50+ problems",         earned: solvedCount >= 50,       progress: Math.min((solvedCount / 50) * 100, 100),       rarity: "legendary" },
    { icon: "🔬", title: "Logic Explorer",  desc: "Open 10+ different topics",  earned: topicEntries.length >= 10, progress: Math.min((topicEntries.length / 10) * 100, 100), rarity: "rare" },
    { icon: "💎", title: "Perfect Score",   desc: "100% accuracy in a session", earned: solvedCount > 0 && solvedCount === attemptedCount, progress: solvedCount > 0 ? Math.round((solvedCount / Math.max(attemptedCount, 1)) * 100) : 0, rarity: "legendary" },
  ];
  ...
  // ── Notifications (built once data loads) ───────────────────────────────────
  useEffect(() => {
    if (!loadingProgress && progressData) {
      setNotifications(buildNotifications(summary, topicStats, badges));
    }
  ...
  const TABS = ["overview", "analytics", "skills", "activity", "achievements", "engagement", "saved"];
      ...
      <main className="pd-main">
          ...
          <div className="pd-hero-left">
            ...
            <div className="pd-hero-info">
              <div className="pd-hero-badges-row">
                ...
                {streakCurrent >= 3 && (
                  <span className="pd-status-badge pd-status-badge--fire">🔥 {streakCurrent}-day streak</span>
                )}
              </div>
            ...
            {loadingProgress ? (
              ...
              <div className="pd-stats-grid">
                ...
                <StatCard
                  ...
                  value={solvedCount}
                  sub={summary.totalProblems ? `of ${summary.totalProblems} total` : "keep going!"}
                  accent={COLORS.green}
                />
                <StatCard
                  icon="⚡"
                  ...
                  sub={solvedCount > 0 ? `${Math.round((solvedCount / attemptedCount) * 100)}% solve rate` : "no attempts yet"}
                  accent={COLORS.blue}
                />
                <StatCard
                  icon="📚"
                  ...
                  sub={summary.totalTopics ? `of ${summary.totalTopics} topics` : "modules finished"}
                ...
                <StatCard
                  ...
                  sub={summary.totalProblems ? `${solvedCount} of ${summary.totalProblems} problems` : "problems solved"}
                  accent={COLORS.pink}
                />
                {/* Row 2 */}
                <StatCard
                  ...
                  sub={streakCurrent > 0 ? `Longest: ${streakLongest}d` : "Start studying today!"}
                  accent={COLORS.amber}
                />
                <StatCard
                  icon="📅"
                  label="Active Days"
                  value={activeDays}
                  sub={activeDays > 0 ? `Longest streak: ${streakLongest}d` : "no activity yet"}
                  accent={COLORS.cyan}
                />
                <StatCard
                  icon="📆"
                ...
                {loadingProgress ? (
                  ...
                  <div className="pd-insight-list">
                    ...
                    <div className="pd-insight-row">
                      ...
                      <span className={`pd-insight-val ${weekComp.delta >= 0 ? "pd-insight-val--green" : "pd-insight-val--amber"}`}>
                    ...
                    <div className="pd-insight-row">
                      ...
                      <span className="pd-insight-val pd-insight-val--purple">
                        {Object.entries(topicStats).filter(([, v]) => v > 0).sort((a, b) => b[1] - a[1])[0]
                          ? Object.entries(topicStats).filter(([, v]) => v > 0).sort((a, b) => b[1] - a[1])[0][0].replace(/([A-Z])/g, " $1").trim()
                          : "—"}
                      </span>
                    </div>
                    <div className="pd-insight-row">
                      ...
                      <span className="pd-insight-val pd-insight-val--amber">
                        {Object.entries(topicStats).filter(([, v]) => v < 100).sort((a, b) => a[1] - b[1])[0]
                          ? Object.entries(topicStats).filter(([, v]) => v < 100).sort((a, b) => a[1] - b[1])[0][0].replace(/([A-Z])/g, " $1").trim()
                          : "All areas strong 🎉"}
                      </span>
                    </div>
                    <div className="pd-insight-row">
                      <span className="pd-insight-label">Topics In Progress</span>
                      <span className="pd-insight-val pd-insight-val--blue">
                        {topicEntries.filter(([, t]) => t.status === "in_progress").length}
                      </span>
                    </div>
                    <div className="pd-insight-row">
              ...
              <div className="pd-card">
                ...
                {loadingProgress ? (
                  ...
                  <div className="pd-overview-empty">
                    ...
                    <span className="pd-overview-empty-msg">No activity yet</span>
                    <span className="pd-overview-empty-sub">Start solving problems to see your activity here.</span>
                    <Link to="/problems" className="pd-btn pd-btn--primary pd-btn--sm" style={{ marginTop: "0.5rem" }}>
                      Go to Problems →
                    </Link>
                  </div>
                ) : (
        ...
        {activeTab === "analytics" && (
          <div className="pd-section">
            ...
            <div className="pd-card">
              ...
              <p className="pd-card-sub">Problems solved and attempted per week</p>
              {weeklyTrend.length === 0 ? (
                <p className="pd-empty">No data yet — start learning to see trends!</p>
              ) : (
                <ResponsiveContainer width="100%" height={240}>
                  <LineChart data={weeklyTrend} margin={{ top: 8, right: 16, left: 0, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="rgba(148,163,184,0.15)" />
                    <XAxis dataKey="week" tick={{ fontSize: 12, fill: "var(--secondary-text)" }} />
                    <YAxis tick={{ fontSize: 12, fill: "var(--secondary-text)" }} allowDecimals={false} />
                    <Tooltip content={<ChartTooltip />} />
                    <Legend wrapperStyle={{ fontSize: "0.82rem" }} />
                    <Line type="monotone" dataKey="solved"   stroke={COLORS.green}  strokeWidth={2.5} dot={{ r: 4 }} name="Solved" />
                    <Line type="monotone" dataKey="attempts" stroke={COLORS.blue}   strokeWidth={2.5} dot={{ r: 4 }} name="Attempts" />
                  </LineChart>
                </ResponsiveContainer>
              )}
            </div>
            ...
            <div className="pd-two-col">
              <div className="pd-card">
                ...
                <p className="pd-card-sub">Daily breakdown of learning actions</p>
                {dailyActivity.length === 0 ? (
                  <p className="pd-empty">No recent activity.</p>
                ) : (
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart data={dailyActivity} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="rgba(148,163,184,0.15)" />
                      <XAxis dataKey="day" tick={{ fontSize: 12, fill: "var(--secondary-text)" }} />
                      <YAxis tick={{ fontSize: 12, fill: "var(--secondary-text)" }} allowDecimals={false} />
                      <Tooltip content={<ChartTooltip />} />
                      <Legend wrapperStyle={{ fontSize: "0.82rem" }} />
                      ...
                      <Bar dataKey="topics"   fill={COLORS.purple} name="Topics"   radius={[4, 4, 0, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                )}
              </div>
              ...
              <div className="pd-card">
                ...
                <p className="pd-card-sub">Completion % across all topic areas</p>
                <ResponsiveContainer width="100%" height={220}>
                  <RadarChart data={radarData} margin={{ top: 8, right: 24, left: 24, bottom: 8 }}>
                    <PolarGrid stroke="rgba(148,163,184,0.2)" />
                    <PolarAngleAxis dataKey="subject" tick={{ fontSize: 11, fill: "var(--secondary-text)" }} />
                    <PolarRadiusAxis angle={30} domain={[0, 100]} tick={{ fontSize: 10, fill: "var(--secondary-text)" }} />
                    <Radar name="Completion %" dataKey="A" stroke={COLORS.blue} fill={COLORS.blue} fillOpacity={0.25} strokeWidth={2} />
                    <Tooltip content={<ChartTooltip />} />
                  </RadarChart>
                </ResponsiveContainer>
              </div>
            ...
            <div className="pd-two-col">
              <div className="pd-card">
                ...
                <p className="pd-card-sub">Proportion of completion per subject area</p>
                {pieData.length === 0 ? (
              ...
              <div className="pd-card">
                ...
                <div className="pd-week-compare">
                  <div className="pd-week-col">
                    ...
                    <span className="pd-week-val pd-week-val--blue">{weekComp.thisWeek}</span>
                    <span className="pd-week-sub">actions</span>
                  </div>
                  <div className={`pd-week-delta ${weekComp.delta >= 0 ? "pd-week-delta--up" : "pd-week-delta--down"}`}>
                ...
                <div className="pd-insight-list" style={{ marginTop: "1rem" }}>
                  <div className="pd-insight-row">
                    ...
                    <span className="pd-insight-val pd-insight-val--purple">{mostActiveDay}</span>
                  </div>
                  <div className="pd-insight-row">
                    <span className="pd-insight-label">Longest Streak</span>
                    <span className="pd-insight-val pd-insight-val--blue">{streakLongest} days</span>
                  </div>
                  <div className="pd-insight-row">
                    <span className="pd-insight-label">Current Streak</span>
                    <span className="pd-insight-val pd-insight-val--green">{streakCurrent} days 🔥</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        {activeTab === "skills" && (
          <div className="pd-section">
                  ...
                  <ul className="pd-topic-list">
                    {topicEntries.slice(0, 10).map(([id, t]) => (
                      <li key={id} className="pd-topic-item">
                        ...
                        <div className="pd-skill-track">
                          <div className="pd-skill-fill" style={{ width: `${t.completionPercentage || 0}%`, background: t.status === "completed" ? COLORS.green : COLORS.blue }} />
                        </div>
            ...
            <div className="pd-card">
              ...
              <p className="pd-card-sub">Completion percentage per subject area</p>
              <ResponsiveContainer width="100%" height={220}>
                <BarChart
                  data={[
          ...
          <div className="pd-section">
            <div className="pd-card">
              ...
              <div className="pd-cal-stats">
                ...
                <span>📆 Most active: <strong>{mostActiveDay}</strong></span>
              </div>
            </div>
            ...
            <div className="pd-card">
              ...
              <p className="pd-card-sub">Your activity history in chronological order</p>
              {loadingProgress ? (
                ...
                <div className="pd-timeline">
                  {recentEvents.map((ev, idx) => {
                    const meta = EVENT_META[ev.type] || { label: ev.type, color: "#94a3b8", icon: "•" };
                    return (
                      <div key={ev.id || ev.createdAt} className="pd-timeline-item">
                        <div className="pd-timeline-line" style={{ background: idx === recentEvents.length - 1 ? "transparent" : "var(--border-color)" }} />
                        <div className="pd-timeline-dot" style={{ background: meta.color }}>{meta.icon}</div>
                        <div className="pd-timeline-content">
                          <span className="pd-timeline-label">{meta.label}</span>
                          {ev.title && <span className="pd-timeline-title">"{ev.title}"</span>}
                          <span className="pd-timeline-time">{timeAgo(ev.createdAt)}</span>
                        </div>
                      </div>
                    );
                  })}
        ...
        {activeTab === "achievements" && (
          <div className="pd-section">
            {/* Summary row */}
            <div className="pd-stats-grid">
              <StatCard icon="🏅" label="Badges Earned"   value={badges.filter(b => b.earned).length} sub={`of ${badges.length} total`} accent={COLORS.amber} />
              <StatCard icon="🌟" label="Legendary"       value={badges.filter(b => b.earned && b.rarity === "legendary").length} sub="legendary badges" accent={COLORS.purple} />
              <StatCard icon="💎" label="Epic"            value={badges.filter(b => b.earned && b.rarity === "epic").length}      sub="epic badges"      accent={COLORS.blue} />
              <StatCard icon="🔥" label="Streak Record"   value={`${streakLongest}d`} sub="longest streak" accent={COLORS.amber} />
            </div>

            <div className="pd-card pd-card--transparent">
              <div className="pd-achievements-header">
                <h2 className="pd-card-title">Trophy Cabinet</h2>
                <p className="pd-card-sub">Your earned accolades and progress toward new ranks</p>
              </div>
        ...
        {activeTab === "engagement" && (
          <div className="pd-section">
            ...
            {/* ── Top KPI row ── */}
            <div className="pd-stats-grid">
              <StatCard icon="📅" label="Active Days"    value={activeDays}                              sub="total days studied"       accent={COLORS.blue} />
              <StatCard icon="🔥" label="Current Streak" value={`${streakCurrent}d`}                    sub={`Best: ${streakLongest}d`} accent={COLORS.amber} />
              <StatCard icon="🏅" label="Badges Earned"  value={badges.filter(b => b.earned).length}    sub={`of ${badges.length} total`} accent={COLORS.purple} />
              <StatCard icon="🎯" label="Accuracy"       value={solvedCount > 0 ? `${Math.round((solvedCount / Math.max(attemptedCount, 1)) * 100)}%` : "—"} sub="solve rate" accent={COLORS.green} />
            </div>
            ...
            <div className="pd-two-col">
              ...
              <div className="pd-card">
                <div className="pd-eng-card-header">
                  ...
                  <span className="pd-eng-notif-count" style={{ background: COLORS.amber }}>3</span>
                </div>
                <ul className="pd-eng-notif-list">
                  <li className="pd-eng-notif-item pd-eng-notif-item--achievement">
                    ...
                    <div className="pd-eng-notif-body">
                      <span className="pd-eng-notif-msg">Maintain a 3-day streak</span>
                      <span className="pd-eng-notif-time">{streakCurrent}/3 Days</span>
                    </div>
                  </li>
                  <li className="pd-eng-notif-item pd-eng-notif-item--info">
                    ...
                    <div className="pd-eng-notif-body">
                      ...
                      <span className="pd-eng-notif-time">{Math.min(solvedCount, 5)}/5 Solved</span>
                    </div>
                  </li>
                  <li className="pd-eng-notif-item pd-eng-notif-item--badge">
                    ...
                    <div className="pd-eng-notif-body">
                      <span className="pd-eng-notif-msg">Start a new topic</span>
              ...
              <div className="pd-card">
                <div className="pd-eng-card-header">
                  <div>
                    ...
                    <p className="pd-card-sub" style={{ margin: 0 }}>Help us improve the platform</p>
                  </div>
                ...
                {feedbackDone ? (
                  <div className="pd-eng-feedback-done">
                    ...
                    <span className="pd-eng-feedback-done-title">Thank you!</span>
                    <span className="pd-eng-feedback-done-sub">Your feedback helps us build a better learning experience.</span>
                  </div>
                ) : (
                  <FeedbackWidget onSubmit={({ rating, comment }) => {
            ...
            <div className="pd-card pd-card--transparent pd-mastery-container">
              ...
              <p className="pd-card-sub">Professional credentials earned through topic mastery</p>
              <div className="pd-cert-grid">
                {[
                  ...
                  <div key={cert.id} className={`pd-cert-card ${cert.unlocked ? "pd-cert-card--unlocked" : "pd-cert-card--locked"}`} style={{ "--cert-color": cert.color }}>
                    ...
                    <div className="pd-cert-content">
                      ...
                      <div className="pd-cert-progress-wrapper">
                        <div className="pd-cert-progress-bar">
                          <div className="pd-cert-progress-fill" style={{ width: `${cert.progress}%`, background: cert.color }}></div>
                        </div>
                        <span className="pd-cert-progress-text">{cert.unlocked ? "Certified" : `${cert.progress}%`}</span>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="pd-eng-tips-grid">
                {[
                  ...
                  },
                  {
                    icon: "🎯",
                    color: COLORS.blue,
                    title: streakCurrent > 0 ? `Keep your ${streakCurrent}-day streak!` : "Start a streak",
                    tip: streakCurrent > 0
                  ...
                  },
                  {
                    ...
                    color: COLORS.purple,
                    title: "Weakest Area",
                    tip: `Focus on ${Object.entries(topicStats).sort((a,b) => a[1]-b[1])[0]?.[0]?.replace(/([A-Z])/g," $1").trim() || "your weakest topic"} to balance your skill profile.`,
                  ...
                  <div key={tip.title} className="pd-eng-tip-card" style={{ "--tip-color": tip.color }}>
                    ...
                    <Link to={tip.path} className="pd-eng-tip-action">{tip.action} →</Link>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="pd-section">
            <div className="pd-card">
              <div className="pd-saved-header">
                ...
                <div className="pd-saved-actions">
                  <button type="button" className="pd-btn pd-btn--primary pd-btn--sm" onClick={() => navigate("/boolforge")}>+ New Circuit</button>
                  <button type="button" className="pd-btn pd-btn--ghost pd-btn--sm" onClick={() => navigate("/kmapgenerator")}>+ New K-Map</button>
                </div>
              </div>
              <div className="pd-saved-grid">
                <div className="pd-saved-card" onClick={() => navigate("/boolforge")}>
                  <div className="pd-saved-thumb pd-saved-thumb--circuit"><span>⚡</span></div>
                ...
                <div className="pd-saved-card" onClick={() => navigate("/kmapgenerator")}>
                  <div className="pd-saved-thumb pd-saved-thumb--kmap"><span>🗺️</span></div>
                  <div className="pd-saved-info">
                ...
                <div className="pd-saved-card" onClick={() => navigate("/problems")}>
                  <div className="pd-saved-thumb pd-saved-thumb--problems"><span>📝</span></div>
                  <div className="pd-saved-info">
            ...
            <div className="pd-card">
              ...
              <div className="pd-sys-grid">
                <div className="pd-sys-row"><span className="pd-sys-label">Session</span><span className="pd-sys-val pd-sys-val--ok">JWT Active</span></div>
                ...
                <div className="pd-sys-row"><span className="pd-sys-label">Last Activity</span><span className="pd-sys-val">{lastLogin}</span></div>
                <div className="pd-sys-row"><span className="pd-sys-label">Account Created</span><span className="pd-sys-val">{joinDate}</span></div>
                <div className="pd-sys-row"><span className="pd-sys-label">Email</span><span className="pd-sys-val">{user?.email}</span></div>
                <div className="pd-sys-row"><span className="pd-sys-label">Role</span><span className="pd-sys-val">Student</span></div>
              </div>
            </div>
          </div>
        )}
